...
'''The code was provided by https://github.com/asiddhant/Active-NLP and adapted by Sophie Burkhardt to our evaluation setting'''
class Loader(object):
    ...
    def load_tweets(self, datapath, pretrained, word_dim = 100,n_splits=5,split_id=0):


        trainpath = os.path.join(datapath, 'res.txt')
        ...
        for train_index, test_index in sss.split(train_data):
            sp_id+=1
            if sp_id==split_id:
                test_data = [train_data[i] for i in test_index]
                ...
                print(
                    '---------------------------------------------------------------------Found %i pretrained embeddings.' % count)
