...
class Acquisition_CLS(object):
    ...
    def __init__(self, train_data, acq_mode='d', init_num=100, seed=9, usecuda = True):
        self.sentencelen = len(train_data)
        self.train_index = set()
        self.npr = np.random.RandomState(seed)
        self.obtain_data(train_data, acquire = init_num)
    ...
    def get_mnlp(self, dataset, trained_model, num_sentences, batch_size = 32):
        
        model = trained_model#= torch.load(model_path) changed !!!
        model.train(False)
        tm = time.time()
    ...
    def obtain_data(self, data, trained_model=None, model_name=None, acquire=2, method='random', num_samples=100):
        
        num_sentences = acquire#(acquire*self.sentencelen)/100
        
        if trained_model is None or model_name is None:
            method = 'random'
        ...
        if method=='random':
            ...
            if self.acq_mode == 'd':
                if method=='mnlp':
                    self.get_mnlp(data, trained_model, num_sentences)#This was changed
                else:
